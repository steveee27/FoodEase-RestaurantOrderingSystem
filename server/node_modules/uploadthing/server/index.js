 ...
 */ const downloadFiles = (urls, downloadErrors)=>Effect.forEach(urls, (_url, idx)=>Effect.gen(function*() {
            ...
            const arrayBuffer = yield* HttpClientRequest.get(url).pipe(HttpClientRequest.modify({
                headers: {}
            }), httpClient.execute, Effect.flatMap((_)=>_.arrayBuffer), Effect.mapError((error)=>{
                downloadErrors[idx] = UploadThingError.toObject(new UploadThingError({
                    code: "BAD_REQUEST",
...
class UTApi {
    constructor(opts){
        ...
        this.executeAsync = async (program, signal)=>{
            const result = await program.pipe(Effect.withLogSpan("utapi.#executeAsync"), (e)=>this.runtime.runPromise(e, signal ? {
                    signal
                } : undefined));
